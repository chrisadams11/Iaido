...
Elm.Globals.make = function (_elm) {
   ...
   var unsafe = function (x) {
      ...
      if (_p0.ctor === "Just") {
            ...
            return _U.crashCase("Globals",
            {start: {line: 26,column: 12},end: {line: 28,column: 54}},
            _p0)("unsafe operation failed!");
         }
   };
   ...
   var Vector = F2(function (a,b) {    return {x: a,y: b};});
...
Elm.Input.make = function (_elm) {
   ...
   var _op = {};
   var playerInputFrame = F6(function (playerID,
   up,
   down,
   left,
   right,
   attackBtn) {
      ...
      var moveKeys = {x: x,y: y};
      return {playerID: playerID
             ,moveKeys: moveKeys
             ,attackBtn: attackBtn};
   });
   var processFrameInput = F2(function (keys,inputSource) {
      return A6(playerInputFrame,
      inputSource.playerID,
      A2($Set.member,inputSource.upKey,keys),
      A2($Set.member,inputSource.downKey,keys),
      A2($Set.member,inputSource.leftKey,keys),
      A2($Set.member,inputSource.rightKey,keys),
      A2($Set.member,inputSource.attackKey,keys));
   });
   var processFrameInputs = F2(function (inputSources,keys) {
      return A2($List.map,processFrameInput(keys),inputSources);
   });
   var updatePlayerInputState = F2(function (playerInputFrame,
   oldState) {
      return {playerID: oldState.playerID
             ,moveDirection: playerInputFrame.moveKeys
             ,attack: playerInputFrame.attackBtn};
   });
   var updateInputState = F2(function (inputFrame,oldState) {
      return {playerInputStates: A3($List.map2,
   ...
   var inputSources = _U.list([{playerID: 1
                               ,upKey: 1
                               ,downKey: 2
                               ,leftKey: 3
                               ,rightKey: 4
                              ...
                              ,{playerID: 2
                               ...
                               ,rightKey: 9
                               ,attackKey: 10}]);
   var startingInputState = {playerInputStates: _U.list([])
                            ,delta: 0};
   var InputSource = F6(function (a,b,c,d,e,f) {
      return {playerID: a
             ,upKey: b
             ,downKey: c
             ,leftKey: d
   ...
   var PlayerInputFrame = F3(function (a,b,c) {
      return {playerID: a,moveKeys: b,attackBtn: c};
   });
   var InputFrame = F2(function (a,b) {
      return {playerInputs: a,delta: b};
   });
   var inputFrameStream = A2($Signal.sampleOn,
   delta,
   A3($Signal.map2,
   InputFrame,
   A2($Signal.map,
   processFrameInputs(inputSources),
   $Keyboard.keysDown),
   delta));
   var inputStateStream = A3($Signal.foldp,
   updateInputState,
   startingInputState,
   inputFrameStream);
   var PlayerInputState = F3(function (a,b,c) {
      return {playerID: a,moveDirection: b,attack: c};
   });
   var InputState = F2(function (a,b) {
      return {playerInputStates: a,delta: b};
   });
   return _elm.Input.values = {_op: _op
                              ...
                              ,playerInputFrame: playerInputFrame
                              ,processFrameInputs: processFrameInputs
                              ,processFrameInput: processFrameInput
                              ,inputFrameStream: inputFrameStream
                              ,inputStateStream: inputStateStream};
};
Elm.Game = Elm.Game || {};
Elm.Game.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var compareTiles = F2(function (l,r) {
      return _U.cmp(l.position.row,
      r.position.row) > 0 ? $Basics.GT : _U.cmp(l.position.row,
      r.position.row) < 0 ? $Basics.LT : _U.cmp(l.position.col,
      r.position.col) > 0 ? $Basics.GT : _U.cmp(l.position.col,
      r.position.col) < 0 ? $Basics.LT : $Basics.EQ;
   });
   var findRepeatTiles = function (l) {
      if (_U.eq(l,_U.list([]))) return _U.list([]); else {
            var sortedList = A2($List.sortWith,compareTiles,l);
            return $Basics.snd(A3($List.foldl,
            F2(function (elem,_p0) {
               var _p1 = _p0;
               var _p2 = _p1._1;
               return _U.eq(elem,_p1._0) ? {ctor: "_Tuple2"
                                           ,_0: elem
                                           ,_1: A2($List._op["::"],elem,_p2)} : {ctor: "_Tuple2"
                                                                                ,_0: elem
                                                                                ,_1: _p2};
            }),
            {ctor: "_Tuple2"
            ,_0: $Globals.unsafeHead(sortedList)
            ,_1: _U.list([])},
            $Globals.unsafeTail(sortedList)));
         }
   };
   var isWalled = F2(function (tile,_p3) {
      var _p4 = _p3;
      var _p6 = _p4.y;
      var _p5 = _p4.x;
      var clearVertically = _U.eq(_p6,
      1) && $Basics.not(tile.walls.top) || (_U.eq(_p6,
      -1) && $Basics.not(tile.walls.bottom) || _U.eq(_p6,0));
      var clearHorizontally = _U.eq(_p5,
      1) && $Basics.not(tile.walls.right) || (_U.eq(_p5,
      -1) && $Basics.not(tile.walls.left) || _U.eq(_p5,0));
      return $Basics.not(clearHorizontally || clearVertically);
   });
   var movePlayer = function (_p7) {
      var _p8 = _p7;
      var _p10 = _p8._0;
      var _p9 = _p8._1;
      var inputState = _p10.inputState;
      ...
      ,y: inputState.moveDirection.y * -1}) ? _U.update(player,
      {collided: true}) : _U.update(player,{tile: _p9});
   };
   var getTile = F3(function (tiles,row,col) {
      return $List.head(A2($List.filter,
      function (tile) {
         return _U.eq(tile.position.row,
         row) && _U.eq(tile.position.col,col);
      },
      tiles));
   });
   var getPlayerMoveTarget = F2(function (playerWithInput,tiles) {
      var targetCol = playerWithInput.player.tile.position.col + ($Basics.not(playerWithInput.inputState.attack) ? playerWithInput.inputState.moveDirection.y : 0);
      var targetRow = playerWithInput.player.tile.position.row + ($Basics.not(playerWithInput.inputState.attack) ? playerWithInput.inputState.moveDirection.x : 0);
      var targetTile = A3(getTile,tiles,targetRow,targetCol);
      var _p11 = targetTile;
      if (_p11.ctor === "Just") {
            return _p11._0;
         } else {
            return playerWithInput.player.tile;
         }
   });
   var getPlayerMoveTargets = F2(function (playersWithInputs,
   tiles) {
      return A2($List.map,
      function (playerWithInput) {
         return {ctor: "_Tuple2"
                ,_0: playerWithInput
                ,_1: A2(getPlayerMoveTarget,playerWithInput,tiles)};
      },
      playersWithInputs);
   });
   var Tile = F2(function (a,b) {
      return {position: a,walls: b};
   });
   var PlayerWithInput = F2(function (a,b) {
      return {player: a,inputState: b};
   });
   var associatePlayerWithInput = F2(function (playerInputStates,
   player) {
      var foundInputState = $List.head(A2($List.filter,
      function (inputState) {
         return _U.eq(inputState.playerID,player.playerID);
      },
      playerInputStates));
      var _p12 = foundInputState;
      if (_p12.ctor === "Just") {
            return $Maybe.Just(A2(PlayerWithInput,player,_p12._0));
         } else {
            return $Maybe.Nothing;
         }
   });
   var associatePlayersWithInputs = F2(function (playerInputStates,
   players) {
      return A2($List.filterMap,
      function (player) {
         return A2(associatePlayerWithInput,playerInputStates,player);
      },
      players);
   });
   var resolveCollisions = function (moveTargets) {
      resolveCollisions: while (true) {
         var crowdedTiles = findRepeatTiles(A2($List.map,
         $Basics.snd,
         moveTargets));
         if (_U.eq(crowdedTiles,_U.list([]))) return moveTargets; else {
               var newMoveTargets = A3($List.foldr,
               F2(function (_p13,acc) {
                  var _p14 = _p13;
                  var _p17 = _p14._1;
                  var _p16 = _p14._0.player;
                  var _p15 = _p14._0.inputState;
                  return A2($List.member,_p17,crowdedTiles) ? A2($List._op["::"],
                  {ctor: "_Tuple2"
                  ,_0: A2(PlayerWithInput,_U.update(_p16,{collided: true}),_p15)
                  ,_1: _p16.tile},
                  acc) : A2($List._op["::"],
                  {ctor: "_Tuple2",_0: A2(PlayerWithInput,_p16,_p15),_1: _p17},
                  acc);
               }),
               _U.list([]),
               moveTargets);
               var _v6 = newMoveTargets;
               moveTargets = _v6;
   ...
   var movePlayers = function (moveTargets) {
      var correctedMoveTargets = resolveCollisions(moveTargets);
      return A2($List.map,movePlayer,correctedMoveTargets);
   };
   var movePhase = F2(function (inputState,gameState) {
      var playersWithInputs = A2(associatePlayersWithInputs,
      inputState.playerInputStates,
      gameState.players);
      var playerMoveTargets = A2(getPlayerMoveTargets,
      playersWithInputs,
      gameState.board);
      return _U.update(gameState,
      {players: movePlayers(playerMoveTargets)});
   });
   var turnPhases = _U.list([movePhase]);
   var updateGameState = F2(function (input,oldGame) {
      return A3($List.foldl,
      F2(function (turnPhase,gameState) {
         return A2(turnPhase,input,gameState);
      }),
      oldGame,
      turnPhases);
   });
   var Player = F6(function (a,b,c,d,e,f) {
      return {playerID: a
             ,tile: b
             ,iaido: c
             ,collided: d
   ...
   var Hit = {ctor: "Hit"};
   var OK = {ctor: "OK"};
   var GameState = F2(function (a,b) {
      return {board: a,players: b};
   });
   return _elm.Game.values = {_op: _op
                             ...
                             ,associatePlayersWithInputs: associatePlayersWithInputs
                             ,associatePlayerWithInput: associatePlayerWithInput
